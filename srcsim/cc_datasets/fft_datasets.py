...
right_shift = 9 # number of points to left shift trigger_lshift.data by
...
raw_cos = 0.38 * np.sin(2*np.pi * freq * t * t_step ) + 0.5 +np.random.normal(loc=0,scale=0.08,size=256) # generate sine wave
cos_floats = raw_cos*1023
for i in range(len(cos_floats)):
    if cos_floats[i] > 1023:
        cos_floats[i] = 1023
    elif cos_floats[i] < 0:
	    cos_floats[i] = 0
...
plt.plot(decimated)
plt.show()
plt.plot(np.fft.fft((1/64)*decimated[0:len(decimated)]))
plt.show()
reversed = np.flip(decimated)
plt.plot(np.fft.fft((1/64)*reversed[0:len(reversed)]))
plt.show()
